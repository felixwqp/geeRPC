...
func call(registry string) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(i int) {
			defer wg.Done()
			xclient_call_wrapper(xc, context.Background(), "call", "Foo.Sum", &Args{Num1: i, Num2: 2*i})
		}(i)
	}
	wg.Wait()
}
...
func broadcast(registry string) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(i int) {
			defer wg.Done()
			xclient_call_wrapper(xc, context.Background(), "broadcast", "Foo.Sum", &Args{Num1: i, Num2: i * i})
			// expect 2 - 5 timeout
			ctx, _ := context.WithTimeout(context.Background(), time.Second*2)
			xclient_call_wrapper(xc, ctx, "broadcast", "Foo.Sleep", &Args{Num1: i, Num2: i * i})
		}(i)
	}
	wg.Wait()
}
